...
class UnnecessaryMatch extends SyntacticRule("UnnecessaryMatch") {
  override def fix(implicit doc: SyntacticDocument): Patch = {
    val polyFunctionBody = doc.tree.collect { case p: Term.PolyFunction =>
      p.body
    }.collect { case f: Term.Function =>
      f.pos
    }.toSet
    doc.tree.collect {
      case f @ Term.Function.After_4_6_0(
          ...
          ) if p1 == p2 && cases.forall(_.collect {
            case Term.Name(n) if n == p2 => ()
          }.isEmpty) && !polyFunctionBody(f.pos) =>
        Seq(
