...
import fix.TryToEither.FailureLeft
import fix.TryToEither.SuccessRight
...
class TryToEither extends SyntacticRule("TryToEither") {
          ...
          .map { kwMatch =>
            ...
            Seq(
              ...
              if (beforeMatch.is[Token.Dot]) {
                Patch.addRight(kwMatch, "toEither")
              } else {
                Seq(
