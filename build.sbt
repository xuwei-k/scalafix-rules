...
lazy val scala3latest = "3.8.2-RC3"
...
lazy val rules = projectMatrix
  .settings(
    ...
    Test / resourceGenerators += Def.task {
      ...
      IO.writeLines(f, (Compile / externalDependencyClasspath).value.map(_.data).map(fileConverter.value.toPath).map(_.toFile.getCanonicalPath))
      Seq(f)
    }.taskValue,
    Test / resourceGenerators += Def.task {
...
val scalafixRulesDependency = "com.github.xuwei-k" %% "scalafix-rules" % "0.6.23" % Test
...
lazy val inputOutputCommon = Def.settings(
  ...
  libraryDependencies += "org.mockito" % "mockito-subclass" % "5.21.0",
